 ...
 * Story 4.4: Filter by Status - AC#6 Empty Filter Results
 *
 ...
 * Story 4.4 AC#6: When filtering, shows appropriate message with clear option
 */
'use client';

import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { FileText, Filter, Inbox, SearchX } from 'lucide-react';
...
export interface LeadTableEmptyProps {
  ...
  searchTerm?: string;
  /** Whether filters are active (Story 4.4 AC#6) */
  hasFilters?: boolean;
  /** Callback to clear search/filters (Story 4.3 AC#6, Story 4.4 AC#6) */
  onClearSearch?: () => void;
}
...
export function LeadTableEmpty({
  ...
  // Story 4.3 AC#6, Story 4.4: Show filter-specific empty state
  if (searchTerm || hasFilters) {
    const hasSearchAndFilter = searchTerm && hasFilters;
    const titleText = searchTerm
      ? `No leads found for "${searchTerm}"`
      : 'No leads match the selected filters';
    const descriptionText = hasSearchAndFilter
      ? 'Try different keywords or adjust your filters'
      : searchTerm
        ? 'Try a different search term or clear the search to see all leads'
        : 'Try selecting different statuses or clear filters to see all leads';
    const buttonText = hasSearchAndFilter
