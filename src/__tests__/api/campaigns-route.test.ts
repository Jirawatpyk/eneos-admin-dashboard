...
import { GET } from '@/app/api/admin/campaigns/route';
...
function createRequest(params: Record<string, string> = {}) {
  const url = new URL('http://localhost/api/admin/campaigns');
...
describe('GET /api/admin/campaigns', () => {
  ...
  it('should forward backend error status', async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: false, status: 503,
      json: async () => ({ success: false, error: { code: 'SERVICE_UNAVAILABLE' } }),
    });
