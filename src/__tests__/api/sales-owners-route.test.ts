...
import { GET } from '@/app/api/sales-owners/route';
...
describe('GET /api/sales-owners', () => {
  ...
  it('should forward request with Bearer token', async () => {
    mockSession('owners-jwt');
    mockFetch.mockResolvedValue({
      ok: true, status: 200,
      json: async () => ({ success: true, data: { team: [{ id: '1', name: 'Sales A' }] } }),
    });
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: 'Bearer owners-jwt',
        }),
      })
    );
  });

  it('should forward backend error with standard format', async () => {
    mockSession();
    mockFetch.mockResolvedValue({
      ok: false, status: 503,
